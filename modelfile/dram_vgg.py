...
class SAF(chainer.Chain):
    def __init__(self, n_units=256, n_out=0, img_size=112, var=0.18, n_step=2, gpu_id=-1):
        super(SAF, self).__init__(
            ...
            # 入力画像を処理するネットワーク
            context_cnn_1=L.Convolution2D(512, 64, 3, pad=1),  # 64 to 62
            context_cnn_2=L.Convolution2D(64, 64, 3, pad=1),  # 31 to 28
            context_cnn_3=L.Convolution2D(64, 16, 3, pad=1),  # 14 to 12
            context_full=L.Linear(14 * 14 * 16, n_units),
    ...
    def __call__(self, x, target, mode, images):
        self.reset()
        n_step = self.n_step
        ...
        if mode == 1:
            ...
            for i in range(n_step):
                if i + 1 == n_step:
                    xm, lm = self.make_img(images, l, num_lm, random=1)
            ...
            for i in range(n_step):
                if i + 1 == n_step:
                    xm, lm = self.make_img(images, l, num_lm, random=0)
    ...
    def first_forward(self, x, num_lm):
        ...
        h2 = F.relu(self.l_norm_cc1(self.context_cnn_1(x)))
        h3 = F.relu(self.l_norm_cc2(self.context_cnn_2(h2)))
        h4 = F.relu(self.l_norm_cc3(self.context_cnn_3(h3)))
